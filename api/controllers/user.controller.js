    ...
    User.find({ email: req.body.email }).then((resp) => {
      ...
      bcrypt.compare(req.body.password, resp[0].password, (err,bycriptResp)=>{
        ...
        if(!err){
          if(bycriptResp){ 
            console.log("bycirpt")
            jwt.sign({ userId: resp[0]._id,name:resp[0].name }, jwtSecret, (err, token)=>{
                console.log("sign",err)
              if(!err){
                console.log("No   Error")
                res.header("Authorization", token)
                res.status(200).json({success:true, message: "Success Login",user:{id:resp[0]._id,user:resp[0].name, token}})
              }
            })
          }else{
